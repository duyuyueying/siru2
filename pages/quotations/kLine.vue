...
function splitData(data, rawData) {
      var categoryData = [];
      var values = [];
      var volumes = [];
      for (var i = 0; i < rawData.length; i++) {
          volumes.push([i, rawData[i], data[i][0] > data[i][1] ? 1 : -1]);
      }
...
volumes = splitData(data, volumes);
		...
		methods: {
			...
			changeOption() {
				...
				data.forEach((item, index) => {
					volumes.push(Math.random() * 200000);
					data.splice(index, 1, [Math.random() * 20000, Math.random() * 20000, Math.random() * 20000, Math.random() * 20000, Math.random() * 200000])
				})
				this.option.series[0].data = this.splitData(data, volumes);
			},
			
			splitData(data, rawData) {
			      var categoryData = [];
			      var values = [];
			      var volumes = [];
				  console.log('inner');
